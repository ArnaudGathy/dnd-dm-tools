...
export const getInitiativeModifier = (character: Character & { capacities: Capacity[] }) => {
  ...
  const hasAlertFeat = !!character.capacities.find(
    ({ name }) => name.toLowerCase().includes("vigilant") || name.toLowerCase().includes("alert"),
  );
  const alertModifier = hasAlertFeat ? PROFICIENCY_BONUS_BY_LEVEL[character.level] : 0;
